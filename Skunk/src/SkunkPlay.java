					...
					while (gameRun) {
							...
							for (int n = 0; n < allPlayerNumber; n++) {
								...
								if (playerChip[n] > 0) {
									...
									for (int num = 0; num < allPlayerNumber; num++) {
										System.out
												.println(playerName[num] + ", Your total score is " + playerScore[num]);

										System.out
												.println(playerName[num] + ", You have " + playerChip[num] + " chips.");
									}
									if (playerChip[n] <= 0) {
										System.out.println(
												"Sorry, player " + playerName[n] + " you lose all your chips.");
									...
									if (playerScore[n] >= WINNERSCORE && playerChip[n] > 0) {
										for (int lastRun = n + 1; lastRun < allPlayerNumber; lastRun++) {
											if (playerChip[lastRun] > 0) {
												...
												System.out.println(playerName[lastRun] + ", Your total score is "
														+ playerScore[lastRun]);
												playerChip[lastRun] = playerChip[lastRun] + playerCurrentTerm[1];
												System.out.println(playerName[lastRun] + ", You have "
														+ playerChip[lastRun] + " chips.");
												kittyBank = kittyBank - playerCurrentTerm[1];
												System.out.println("Now in the bank there is " + kittyBank + " chips.");
												if (playerChip[lastRun] <= 0) {
													System.out.println("Sorry, player " + playerName[lastRun]
															+ " you lose all your chips.");
													System.out.println("You lose");
													System.out.println("--------------------");
												}
											}
										...
										for (int finalWinner = 0; finalWinner < allPlayerNumber; finalWinner++) {
											if (playerScore[finalWinner] >= WINNERSCORE) {
												System.out.println("Congratulations!" + playerName[finalWinner]
														+ " You win the game!");
										...
										for (int r = 0; r < allPlayerNumber; r++) {
											if (playerScore[r] >= WINNERSCORE) {
												System.out.println(
														"Winner get " + winnerGetChip + " chips in the kittybank.");
												System.out.println("-----Winner---------");
												playerChip[r] = playerChip[r] + winnerGetChip;
												System.out.println("Player " + playerName[r] + " ,your final score is "
														+ playerScore[r]);
												System.out.println("Player " + playerName[r] + " ,you have "
														+ playerChip[r] + " chips.");
												System.out.println("---------------------");
